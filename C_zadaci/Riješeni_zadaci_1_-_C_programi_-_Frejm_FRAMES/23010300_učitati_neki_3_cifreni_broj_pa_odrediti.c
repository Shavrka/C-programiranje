/*
 * ZADATAK 23010300 - 23010300
 * Učitati neki 3 cifreni broj pa odrediti i Ispisati njemu najbliži prost broj.
 * Kategorija: Riješeni zadaci 1 - C programi - Frejm /FRAMES/
 * Izvor: https://www.znanje.org/knjige/computer/c/v/230c/23010300.htm
 */
...
{
int x, i;
int xgore = 0, xdole = 0;   // gornji i donji prost trocifren broj

bool prost=true;  // pocetna vrijednost indikatora prostih brojeva

printf("Ucitati trocifren broj: ");
scanf("%d",&x);
while (x < 100 || x > 999) {
printf("Broj nije trocifren. Unesi trocifren broj:: ");
scanf("%d",&x);
}

for (int y = x + 1; y < 1000; y++) {  // trazi se prvi prost broj na vise
